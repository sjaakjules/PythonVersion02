...
class GUI_Manager:
    ...
    def draw(self):
        ...
        if config.IN_DEBUG_MODE:
            debug_surface = config.FONTS['debug'].render(self.debug_text, True, #f400e0 )
    ...
    def draw_selection_screen(self, surface):
        ...
        # Draw title
        title = FONTS['title'].render("Select Game Board", True, (0, 0, 0))
        ...
        for i, rect in enumerate(self.thumbnail_rects):
            ...
            y = padding + row * (THUMBNAIL_SIZE[1] + padding + 40) + 60  # Extra space for title
            ...
            # Draw selection text
            text = FONTS['selection'].render("Click to select", True, (0, 0, 0))
            text_rect = text.get_rect(
                centerx=x + THUMBNAIL_SIZE[0] // 2,
                top=y + THUMBNAIL_SIZE[1] + 10
            )
            surface.blit(text, text_rect)

    def draw_player_areas(self, surface, x, y, scale):
        ...
        pygame.draw.rect(surface, PLAYER_COLORS['p1'], 
        ...
        pygame.draw.rect(surface, PLAYER_COLORS['p2'], 
                        (atlas_x, atlas_y, card_height, card_width), 2)  # Swapped width/height for rotation
    ...
    def draw_grid(self, surface, x, y):
        ...
        for row in range(SQUARE_GRID[1] + 1):
            # Horizontal lines
            start_x = x + self.scaled_offset[0]
            end_x = x + self.scaled_offset[0] + (SQUARE_GRID[0] * self.scaled_tile_size)
            ...
            # Vertical lines
            if row < SQUARE_GRID[1]:
                for col in range(SQUARE_GRID[0] + 1):
                    line_x = x + self.scaled_offset[0] + (col * self.scaled_tile_size)
                    start_y = y + self.scaled_offset[1]
                    end_y = y + self.scaled_offset[1] + (SQUARE_GRID[1] * self.scaled_tile_size)
    ...
    def update_background_size(self, window_size):
